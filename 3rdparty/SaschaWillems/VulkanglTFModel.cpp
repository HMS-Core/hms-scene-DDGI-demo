...
uint32_t vkglTF::descriptorBindingFlags = vkglTF::DescriptorBindingFlags::ImageBaseColor |
										  ...
										  vkglTF::DescriptorBindingFlags::ImagePhysicalMap |
...
// added by Monkey
void vkglTF::Material::createMaterialImageDescriptorSet(VkDescriptorPool descriptorPool,
														VkDescriptorSetLayout descriptorSetLayout,
														uint32_t descriptorBindingFlags,
														vkglTF::Texture* emptyTex)
{
	VkDescriptorSetAllocateInfo descriptorSetAllocInfo{};
	descriptorSetAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	descriptorSetAllocInfo.descriptorPool = descriptorPool;
	...
	// because any texture has been set a real texture or a empty texture in vkglTF::Model::loadMaterials() function,
	// so it's useless to tell if the texture address is empty. 
	//if (baseColorTexture && descriptorBindingFlags & DescriptorBindingFlags::ImageBaseColor) {
	if (baseColorTexture == nullptr) {
		imageDescriptors.push_back(emptyTex->descriptor);
		writeDescriptorSet.dstBinding = static_cast<uint32_t>(writeDescriptorSets.size());
		writeDescriptorSet.pImageInfo = &emptyTex->descriptor;
		writeDescriptorSets.push_back(writeDescriptorSet);
	} else if (descriptorBindingFlags & DescriptorBindingFlags::ImageBaseColor) {
		imageDescriptors.push_back(baseColorTexture->descriptor);
		writeDescriptorSet.dstBinding = static_cast<uint32_t>(writeDescriptorSets.size());
		writeDescriptorSet.pImageInfo = &baseColorTexture->descriptor;
		writeDescriptorSets.push_back(writeDescriptorSet);
	...
	if (normalTexture == nullptr) {
		...
		writeDescriptorSet.pImageInfo = &emptyTex->descriptor;
		writeDescriptorSets.push_back(writeDescriptorSet);
	} else if (descriptorBindingFlags & DescriptorBindingFlags::ImageNormalMap) {
		imageDescriptors.push_back(normalTexture->descriptor);
		writeDescriptorSet.dstBinding = static_cast<uint32_t>(writeDescriptorSets.size());
		writeDescriptorSet.pImageInfo = &normalTexture->descriptor;
		writeDescriptorSets.push_back(writeDescriptorSet);
	...
	if (metallicRoughnessTexture == nullptr) {
		...
		writeDescriptorSet.pImageInfo = &emptyTex->descriptor;
		writeDescriptorSets.push_back(writeDescriptorSet);
	} else if (descriptorBindingFlags & DescriptorBindingFlags::ImagePhysicalMap) {
	...
	if (occlusionTexture == nullptr) {
		...
		writeDescriptorSet.pImageInfo = &emptyTex->descriptor;
		writeDescriptorSets.push_back(writeDescriptorSet);
	} else if (descriptorBindingFlags & DescriptorBindingFlags::ImageOcclusionMap) {
	...
	if (emissiveTexture == nullptr) {
		...
		writeDescriptorSet.pImageInfo = &emptyTex->descriptor;
		writeDescriptorSets.push_back(writeDescriptorSet);
	} else if (descriptorBindingFlags & DescriptorBindingFlags::ImageEmissiveMap) {
...
void vkglTF::Node::update() {
	if (mesh) {
		...
		if (skin) {
			...
			mesh->uniformBlock.jointcount = skin->joints.size();
...
void vkglTF::Model::loadNode(vkglTF::Node* parent, const tinygltf::Node& node, uint32_t nodeIndex, const tinygltf::Model& model, float globalscale)
{
	...
	if (node.mesh > -1) {
		...
		for (size_t j = 0; j < mesh.primitives.size(); j++) {
			...
			uint32_t indexStart = static_cast<uint32_t>(m_indexBuffer.size());
			...
			// Indices
			{
				...
				case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT: {
					...
					for (size_t index = 0; index < accessor.count; index++) {
						...
						// save mesh's self index!
						// save mesh's self index!
						// save mesh's self index!
						m_localIndexBuffer.push_back(buf[index]);
					}
					break;
				}
...
vkglTF::Texture* vkglTF::Model::GetEmptyTexture()
{
	return &emptyTexture;
}
...
void vkglTF::Model::loadMaterials(tinygltf::Model &gltfModel)
{
	for (tinygltf::Material &mat : gltfModel.materials) {
		...
		if (mat.values.find("baseColorTexture") != mat.values.end()) {
			...
			material.baseColorTexture = nullptr; // added by Monkey
		}
		...
		if (mat.values.find("metallicRoughnessTexture") != mat.values.end()) {
			...
			material.metallicRoughnessTexture = nullptr; // added by Monkey
		}
		...
		if (mat.additionalValues.find("normalTexture") != mat.additionalValues.end()) {
			...
			material.normalTexture = nullptr; // added by Monkey
		}
		if (mat.additionalValues.find("emissiveTexture") != mat.additionalValues.end()) {
			...
			material.emissiveTexture = nullptr; // added by Monkey
		}
		if (mat.additionalValues.find("occlusionTexture") != mat.additionalValues.end()) {
			...
			material.occlusionTexture = nullptr; // added by Monkey
		}
...
void vkglTF::Model::loadFromFile(std::string filename, vks::VulkanDevice *device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	if (fileLoaded) {
		...
		indexBuffer.insert(indexBuffer.begin(), m_indexBuffer.begin(), m_indexBuffer.end());
		vertexBuffer.insert(vertexBuffer.begin(), m_vertexBuffer.begin(), m_vertexBuffer.end());
	...
	if (imageCount > 0) {
		...
		if (descriptorBindingFlags & DescriptorBindingFlags::ImagePhysicalMap) {
			poolSizes.push_back({ VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, imageCount });
		}
	...
	// Descriptors for per-material images
	{
		...
		for (auto& material : materials) {
            //if (material.baseColorTexture != nullptr) {
				...
				material.createMaterialImageDescriptorSet(descriptorPool, descriptorSetLayoutImage, descriptorBindingFlags, GetEmptyTexture());
            //}
		}
	}
}
...
void vkglTF::Model::getNodeDimensions(Node *node, glm::vec3 &min, glm::vec3 &max)
{
	if (node->mesh) {
		for (Primitive *primitive : node->mesh->primitives) {
			...
			glm::vec4 locMin = node->getMatrix() * glm::vec4(primitive->dimensions.min, 1.0f);
			glm::vec4 locMax = node->getMatrix() * glm::vec4(primitive->dimensions.max, 1.0f);
			...
			if (locMax.x < min.x) { min.x = locMax.x; }
			if (locMax.y < min.y) { min.y = locMax.y; }
			if (locMax.z < min.z) { min.z = locMax.z; }
			if (locMin.x > max.x) { max.x = locMin.x; }
			if (locMin.y > max.y) { max.y = locMin.y; }
			if (locMin.z > max.z) { max.z = locMin.z; }
		}
	}
	for (auto child : node->children) {
