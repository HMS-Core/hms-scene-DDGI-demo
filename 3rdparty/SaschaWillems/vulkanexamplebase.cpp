...
void VulkanExampleBase::renderLoop()
{
	...
	while (1)
	{
		...
		if (swapChain.prepared)
		{
			...
			if (m_camera.type != SaCamera::CameraType::firstperson)
			{
				// Rotate
				if (std::abs(gamePadState.axisLeft.x) > deadZone)
				{
					m_camera.rotate(glm::vec3(0.0f, gamePadState.axisLeft.x * 0.5f, 0.0f));
					updateView = true;
				}
				if (std::abs(gamePadState.axisLeft.y) > deadZone)
				{
					m_camera.rotate(glm::vec3(gamePadState.axisLeft.y * 0.5f, 0.0f, 0.0f));
					updateView = true;
				}
				// Zoom
				if (std::abs(gamePadState.axisRight.y) > deadZone)
				{
					m_camera.translate(glm::vec3(0.0f, 0.0f, gamePadState.axisRight.y * 0.01f));
					updateView = true;
				}
				if (updateView)
				{
			...
			{
				updateView = m_camera.updatePad(gamePadState.axisLeft, gamePadState.axisRight, frameTimer);
				if (updateView)
				{
					viewChanged();
				}
...
void VulkanExampleBase::updateOverlay()
{
	...
	ImGui::Text("camera rotation:%.2f %.2f %.2f", m_camera.rotation.x, m_camera.rotation.y, m_camera.rotation.z);
...
void VulkanExampleBase::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		if (m_camera.type == SaCamera::firstperson)
		{
			switch (wParam)
			{
			case KEY_W:
