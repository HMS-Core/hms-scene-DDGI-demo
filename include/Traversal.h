...
enum class TraceRayHitFormat {
    ...
    T_PRIMID_INSTID_U_V,
    /* *< ray distance (t<0 for miss), primitive id, instance id, global triangle id, and the barycentric u,v. */
    T_PRIMID_INSTID_GLOBALID_U_V,
};
...
/// @brief The data definition for TraceRayHitFormat::T_PRIMID_INSTID_GLOBALID_U_V format
struct HitDistPrimInstGidCoord {
    float t;
    uint32_t instId;
    uint32_t globalId;
    float u;
...
struct InstanceDescription {
    // The 4x4 affine transform matrix, e.g., rotation, scaling and translation.
    float transform[NUM_MAT][NUM_MAT];
...
/// @brief The RayTracing ShaderModule createInfo description.
    ...
    public:
        ...
        /**
         * Get Bounding Box of whole Scene, call after create BLAS
         */
        Result GetSceneBox(float min[3], float max[3]) const noexcept;

        /**
         * Get Bounding Box of whole Scene, call before create BLAS
         */
        Result GetSceneBox(float min[3], float max[3], uint32_t geometriesCount,
                         const GeometryTriangleDescription *geometries,
                         BLAS *blases) const noexcept;
